# ============================================================
# SCRIPT DE TESTING: SISTEMA JERÁRQUICO DE USUARIOS
# ============================================================
# Prueba la creación de usuarios, jerarquía y permisos
# Bingo 24K - Sistema de Red Multi-nivel
# ============================================================

$BASE_URL = "http://localhost:3000"

Write-Host "🧪 INICIANDO TESTS DEL SISTEMA JERÁRQUICO" -ForegroundColor Cyan
...
try {
    ...
    Write-Host "✅ Login SuperAdmin exitoso" -ForegroundColor Green
...
try {
    ...
    Write-Host "✅ Agente1 creado exitosamente" -ForegroundColor Green
...
try {
    ...
    Write-Host "✅ Login Agente1 exitoso" -ForegroundColor Green
} catch {
...
$agente2Data = @{
    ...
    role = "agente"
    nombre_completo = "María Sub-Agente"
} | ConvertTo-Json
...
try {
    ...
    Write-Host "✅ Sub-Agente creado exitosamente" -ForegroundColor Green
...
try {
    ...
    Write-Host "✅ Jugador creado exitosamente" -ForegroundColor Green
...
try {
    ...
    Write-Host "✅ Login Sub-Agente exitoso" -ForegroundColor Green
} catch {
...
try {
    ...
    Write-Host "✅ Jugador Nivel 3 creado exitosamente" -ForegroundColor Green
...
# ============================================================
# PASO 8: VERIFICAR JERARQUÍA DESDE AGENTE1
# ============================================================
Write-Host "🌲 PASO 8: Verificar jerarquía desde Agente1..." -ForegroundColor Yellow

try {
    $headers = @{ Authorization = "Bearer $AGENTE1_TOKEN" }
    $jerarquiaAgente1 = Invoke-RestMethod -Uri "$BASE_URL/api/admin/users/hierarchy" -Method GET -Headers $headers
    
    Write-Host "✅ Jerarquía de Agente1:" -ForegroundColor Green
    ...
    # Verificar que ve los 3 usuarios creados bajo él
    ...
    if ($idsEncontrados -contains $AGENTE2_ID -and $idsEncontrados -contains $JUGADOR1_ID -and $idsEncontrados -contains $JUGADOR2_ID) {
        Write-Host "   ✅ Agente1 ve TODA su red (3 usuarios)" -ForegroundColor Green
    } else {
        Write-Host "   ⚠️  Advertencia: Agente1 no ve todos los usuarios esperados" -ForegroundColor Yellow
    }
} catch {
    Write-Host "❌ Error obteniendo jerarquía Agente1: $($_.Exception.Message)" -ForegroundColor Red
}

Write-Host ""

# ============================================================
# PASO 9: VERIFICAR JERARQUÍA DESDE SUB-AGENTE
# ============================================================
Write-Host "🌲 PASO 9: Verificar jerarquía desde Sub-Agente..." -ForegroundColor Yellow

try {
    $headers = @{ Authorization = "Bearer $AGENTE2_TOKEN" }
    $jerarquiaAgente2 = Invoke-RestMethod -Uri "$BASE_URL/api/admin/users/hierarchy" -Method GET -Headers $headers
    
    Write-Host "✅ Jerarquía de Sub-Agente:" -ForegroundColor Green
    ...
    if ($idsEncontrados.Count -eq 1 -and $idsEncontrados -contains $JUGADOR2_ID) {
        Write-Host "   ✅ Sub-Agente ve solo SU red (1 usuario)" -ForegroundColor Green
    } else {
        Write-Host "   ⚠️  Sub-Agente ve: $($jerarquiaAgente2.all.Count) usuarios" -ForegroundColor Yellow
    }
} catch {
    Write-Host "❌ Error obteniendo jerarquía Sub-Agente: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    Write-Host "✅ Saldo cargado exitosamente" -ForegroundColor Green
} catch {
...
try {
    ...
    Write-Host "❌ ERROR: Sub-Agente NO debería poder modificar Jugador1" -ForegroundColor Red
} catch {
    if ($_.Exception.Response.StatusCode.Value__ -eq 403) {
        Write-Host "✅ CORRECTO: Sub-Agente bloqueado (403 Forbidden)" -ForegroundColor Green
        Write-Host "   Mensaje: Fuera de su red jerárquica" -ForegroundColor Gray
    } else {
...
# ============================================================
Write-Host "👁️  PASO 12: SuperAdmin ve TODOS los usuarios..." -ForegroundColor Yellow

try {
    $headers = @{ Authorization = "Bearer $SUPERADMIN_TOKEN" }
    $jerarquiaSuperAdmin = Invoke-RestMethod -Uri "$BASE_URL/api/admin/users/hierarchy" -Method GET -Headers $headers
    
    Write-Host "✅ Jerarquía completa de SuperAdmin:" -ForegroundColor Green
    ...
    Write-Host "❌ Error obteniendo jerarquía SuperAdmin: $($_.Exception.Message)" -ForegroundColor Red
}
...
Write-Host "   2. Creación de Agente Nivel 1" -ForegroundColor Gray
...
Write-Host ""
Write-Host "🎯 SISTEMA JERÁRQUICO FUNCIONANDO CORRECTAMENTE" -ForegroundColor Green
Write-Host ""
