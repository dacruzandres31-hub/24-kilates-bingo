...
const [cardWinningLines, setCardWinningLines] = useState({}); // {cardId: [0,1,2]} l�neas ganadoras por cart�n
  ...
  const nextDrawTime = useMemo(() => {
    ...
    drawTime.setHours(21, 0, 0, 0); // Plata a las 21:00
    
    // Si ya pas� las 21:00 hoy, programar para ma�ana
    if (today > drawTime) {
...
celebrationAudio.volume = 0.7;
  ...
  useEffect(() => {
    const checkSalesStatus = async () => {
      try {
        const response = await fetch('/api/game/sales-status/plata', {
  ...
  useEffect(() => {
    ...
    const STORAGE_KEY = `bingo_cards_plata_${activeSessionId}`;
    ...
    const checkExistingCards = async () => {
      ...
      try {
        const response = await fetch(`/api/game/my-cards?roomType=plata`, {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('playerToken') || localStorage.getItem('token')}` }
        });
  ...
  // Funci�n para obtener color seg�n el n�mero - Paleta Plateada con contraste
  const getBallColor = (number) => {
    // Tonos plateados con mejor contraste - alternando claros y oscuros
    if (number >= 1 && number <= 10) return '#8a8a8a'; // Gris oscuro
    if (number >= 11 && number <= 20) return '#b0b0b0'; // Gris medio
    if (number >= 21 && number <= 30) return '#6d6d6d'; // Gris carb�n
    if (number >= 31 && number <= 40) return '#9a9a9a'; // Gris plata oscuro
    if (number >= 41 && number <= 50) return '#787878'; // Gris acero
    if (number >= 51 && number <= 60) return '#a8a8a8'; // Gris claro
    if (number >= 61 && number <= 70) return '#5a5a5a'; // Gris grafito
    if (number >= 71 && number <= 80) return '#888888'; // Gris medio oscuro
    return '#757575'; // Gris neutro (81-90)
  };
  ...
  // PREMIO �NICO: Solo celebrar si lineWinnersPaid === false (premio a�n disponible)
  ...
  if (newWinners.length > 0 && !lineCelebrated && winnerCards.length === 0 && !lineWinnersPaid) {
    ...
    setTimeout(() => {
      console.log('[SilverRoom] ?? Reproduciendo voz: Felicitaciones, Ganaste L�nea');
...
useEffect(() => {
  ...
  useEffect(() => {
    if (gameStatus === 'active' && previousGameStatus === 'waiting') {
      ...
      audioService.startBolilleroGirando();
      // SEGUNDO: Bajar volumen de m�sica de fondo al 70%
      ...
      audioService.stopBolilleroGirando();
      // Restaurar volumen de m�sica al 100%
      ...
      audioService.startBolilleroGirando();
      // Bajar volumen de m�sica de fondo al 70%
      audioService.lowerMusicVolume();
    }
    setPreviousGameStatus(gameStatus);
  }, [gameStatus]);
        ...
        />
          ...
          {showLateEntryModal && (
            <div className="winner-celebration-overlay" style={{ backgroundColor: 'rgba(0,0,0,0.85)' }}>
              <div className="celebration-content" style={{ maxWidth: '400px' }}>
                ...
                <h2 style={{ color: '#C0C0C0', marginBottom: '12px' }}>Premio de Línea Ya Otorgado</h2>
          ...
          {showLateBingoModal && (
            <div className="winner-celebration-overlay" style={{ backgroundColor: 'rgba(0,0,0,0.95)' }}>
              <div className="celebration-content" style={{ maxWidth: '400px' }}>
                <div style={{ fontSize: '48px', marginBottom: '16px' }}>💰</div>
                <h2 style={{ color: '#C0C0C0', marginBottom: '12px' }}>Sorteo Finalizado</h2>
                <p style={{ color: '#fff', fontSize: '16px', lineHeight: '1.5' }}>
                  El <strong style={{ color: '#C0C0C0' }}>BINGO</strong> ya fue ganado por otro jugador.
                  <br /><br />
