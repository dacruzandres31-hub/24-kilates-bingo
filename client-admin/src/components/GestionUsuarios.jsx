...
import ResourceSummary from './users/ResourceSummary';
import UserManagementDetail from './users/UserManagementDetail';
import BlockedUserWarningModal from './users/BlockedUserWarningModal';
import ChangePasswordModal from './users/ChangePasswordModal';
import EditUserModal from './users/EditUserModal';
import CreateUserModal from './users/CreateUserModal';
import OperationConfirmModal from './users/OperationConfirmModal';

export default function GestionUsuarios({ sharedUserData, sharedCartonesStock, onResourcesUpdate }) {
  // ... (existing state)

  // Estado para la calculadora (V2) y Carga Masiva
  ...
  const handleCalculatorApply = (result) => {
    ...
    // Note: We don't close calculator here if we want "Back" functionality, but standard flow suggests closing calc -> opening review.
    setShowCalculator(false);
  };

  const handleBulkSuccess = async (response) => {
    // 1. Refresh user data globally
    // We call API manually here to await the result and update Modal immediately
    try {
      const token = localStorage.getItem('adminToken');
      const res = await axios.get('/api/admin/users/hierarchy', {
      ...
      // Refresh list view (usuariosDelAgente) 
      if (agenteSeleccionado) {
        cargarUsuariosDelAgente(agenteSeleccionado.id, normalizedUsers);
      }

      // 2. Refresh current user view if open
      if (modalGestionUsuario.usuario) {
        const updatedUser = normalizedUsers.find(u => u.id === modalGestionUsuario.usuario.id);
        if (updatedUser) {
          const giftCards = await cargarGiftCards(updatedUser.id);
      ...
      // 3. Refresh Admin Resources (Top Bar & Panels)
      try {
        ...
        if (adminProfileRes.data && adminInventoryRes.data && onResourcesUpdate) {
      ...
      setSuccessMessage('✅ Carga masiva realizada exitosamente');
  ...
  const ejecutarOperacion = async () => {
    ...
    try {
      ...
      if (tipo === 'dinero-cargar' || tipo === 'dinero-descargar') {
        ...
        if (modalGestionUsuario.isOpen && modalGestionUsuario.usuario?.id === userId) {
          const usuarioActualizado = normalizedAll.find(u => u.id === userId);
          if (usuarioActualizado) {
        ...
        // Actualizar vista del agente seleccionado con datos normalizados
        if (agenteSeleccionado) {
          cargarUsuariosDelAgente(agenteSeleccionado.id, normalizedAll);
        }
    ...
    <>
      ...
      <div data-section="usuarios-active">
        ...
        <ResourceSummary
          ...
          onOpenConfirmModal={(tipo, sala) => {
            console.log('🔍 Click en +Recurso, tipo:', tipo, 'sala:', sala);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Panel Izquierdo: Árbol de Referidos - Modularizado */}
        <UserTreeView
          ...
          onOpenCreateModal={abrirModalCrearUsuario}
          tieneHijos={tieneHijos}
          tieneSubAgentes={tieneSubAgentes}
        />
        {/* Panel Derecho: Listado de Usuarios - Modularizado */}
        <UserListView
          agenteSeleccionado={agenteSeleccionado}
          usuariosDelAgente={usuariosDelAgente}
          allUsersHierarchy={allUsersHierarchy}
          busquedaUsuario={busquedaUsuario}
          currentUser={currentUser}
          modalGestionUsuario={modalGestionUsuario}
          onBusquedaChange={setBusquedaUsuario}
          onSelectUser={async (usuario) => {
            if (usuario.is_blocked) {
              setModalUsuarioBloqueado({ isOpen: true, usuario });
            } else {
              const giftCards = await cargarGiftCards(usuario.id);
              setModalGestionUsuario({ isOpen: true, usuario, giftCards });
            }
          }}
          ...
          onOpenBlockModal={(usuario) => setModalBlockUser({ isOpen: true, usuario })}
          onOpenUnblockModal={(usuario) => setModalUnblockUser({ isOpen: true, usuario })}
          tieneSubAgentes={tieneSubAgentes}
        />
      ...
      {/* Modal de Creación de Usuario - Modularizado */}
      {
        modalCrearUsuario.isOpen && (
          <CreateUserModal
            isOpen={modalCrearUsuario.isOpen}
            tipoUsuario={modalCrearUsuario.tipoUsuario}
            isAdmin={modalCrearUsuario.isAdmin}
            tabActiva={modalCrearUsuario.tabActiva}
            datosIngreso={modalCrearUsuario.datosIngreso}
            datosPersonales={modalCrearUsuario.datosPersonales}
            datosBancarios={modalCrearUsuario.datosBancarios}
            showPassword={showPasswordCreate}
            passwordStrength={passwordStrengthCreate}
            onClose={() => setModalCrearUsuario({ ...modalCrearUsuario, isOpen: false })}
            onTipoChange={(tipo) => setModalCrearUsuario({ ...modalCrearUsuario, tipoUsuario: tipo })}
            onAdminChange={(isAdmin) => setModalCrearUsuario({ ...modalCrearUsuario, isAdmin })}
            onTabChange={(tab) => setModalCrearUsuario({ ...modalCrearUsuario, tabActiva: tab })}
            onDatosIngresoChange={(datos) => setModalCrearUsuario({ ...modalCrearUsuario, datosIngreso: datos })}
            onDatosPersonalesChange={(datos) => setModalCrearUsuario({ ...modalCrearUsuario, datosPersonales: datos })}
            onDatosBancariosChange={(datos) => setModalCrearUsuario({ ...modalCrearUsuario, datosBancarios: datos })}
            onPasswordVisibilityToggle={() => setShowPasswordCreate(!showPasswordCreate)}
            onPasswordChange={(pwd) => setPasswordStrengthCreate(calculatePasswordStrength(pwd))}
                ...
                <div>
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <button
                      ...
                      className="text-xs bg-blue-600/30 hover:bg-blue-600/50 text-blue-200 border border-blue-500/50 px-3 py-1.5 rounded-lg flex items-center gap-1 transition-all"
                    >
      ...
      {/* Modal de Información del Usuario - Modularizado */}
      {
        modalInformacion.isOpen && (
          <UserInfoModal
            isOpen={modalInformacion.isOpen}
            usuario={modalInformacion.usuario}
            parent={modalInformacion.parent}
            estructura={modalInformacion.estructura}
            agentesCount={modalInformacion.agentesCount}
            jugadoresCount={modalInformacion.jugadoresCount}
            gamificationStats={modalInformacion.gamificationStats}
            onClose={() => setModalInformacion({ isOpen: false, usuario: null, estructura: [], agentesCount: 0, jugadoresCount: 0, parent: null })}
            onGrantAchievement={handleGrantAchievement}
          />
        )
      }

      {/* Modal de Cambiar Contraseña - Modularizado */}
      {
        modalCambiarPassword.isOpen && (
          <ChangePasswordModal
            isOpen={modalCambiarPassword.isOpen}
            usuario={modalCambiarPassword.usuario}
            newPassword={modalCambiarPassword.newPassword}
            confirmPassword={modalCambiarPassword.confirmPassword}
            showPassword={modalCambiarPassword.showPassword}
            passwordStrength={passwordStrengthChange}
            isProcessing={modalCambiarPassword.isProcessing}
            onClose={() => {
              setModalCambiarPassword({
                isOpen: false,
                usuario: null,
            ...
            onNewPasswordChange={(pwd) => {
              setModalCambiarPassword(prev => ({ ...prev, newPassword: pwd }));
              setPasswordStrengthChange(calculatePasswordStrength(pwd));
            }}
            onConfirmPasswordChange={(pwd) => setModalCambiarPassword(prev => ({ ...prev, confirmPassword: pwd }))}
            onPasswordVisibilityToggle={() => setModalCambiarPassword(prev => ({ ...prev, showPassword: !prev.showPassword }))}
            onConfirm={handleCambiarPassword}
          />
        )
      }

      {/* Modal de Modificar Datos - Modularizado */}
      {
        modalModificar.isOpen && (
          <EditUserModal
            isOpen={modalModificar.isOpen}
            usuario={modalModificar.usuario}
            datosPersonales={modalModificar.datosPersonales}
            datosBancarios={modalModificar.datosBancarios}
            isProcessing={modalModificar.isProcessing}
            onClose={() => {
              setModalModificar({
                isOpen: false,
                usuario: null,
            ...
            onDatosPersonalesChange={(datos) => setModalModificar({ ...modalModificar, datosPersonales: datos })}
            onDatosBancariosChange={(datos) => setModalModificar({ ...modalModificar, datosBancarios: datos })}
            onConfirm={handleModificarUsuario}
          />
        )
      }
      ...
      {/* Modal de Usuario Bloqueado - Modularizado */}
      {
        modalUsuarioBloqueado.isOpen && (
          <BlockedUserWarningModal
            isOpen={modalUsuarioBloqueado.isOpen}
            usuario={modalUsuarioBloqueado.usuario}
            onClose={() => setModalUsuarioBloqueado({ isOpen: false, usuario: null })}
          />
        )
      }
      ...
      {/* Calculator Modal */}
      {
        showCalculator && createPortal(
          <CostCalculatorModal
            ...
            targetRole={
              modalGestionUsuario.usuario?.role === 'jugador' ? 'player' :
                modalGestionUsuario.usuario?.role === 'agente' ? 'agent' : 'admin'
            }
          />,
          document.body
        )
