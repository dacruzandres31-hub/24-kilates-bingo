-- Fix: Usar comparación BINARY para evitar collation issues
...
BEGIN
  ...
  DECLARE cur_cards CURSOR FOR 
    ...
    WHERE user_id = p_from_user_id AND BINARY room = BINARY p_room
  ...
  IF (SELECT COALESCE(SUM(quantity), 0) FROM user_card_inventory 
      WHERE user_id = p_from_user_id AND BINARY room = BINARY p_room) < p_quantity THEN
  ...
  transfer_loop: LOOP
    ...
    WHERE user_id = p_from_user_id AND BINARY room = BINARY p_room AND is_gift = v_is_gift;
...
SELECT 'SP con comparación BINARY recreado' AS status;
