-- Fix: Asegurar que INSERT a card_movements_log use CAST
...
BEGIN
  ...
  transfer_loop: LOOP
    ...
    -- Log salida (CAST room para match con ENUM)
    INSERT INTO card_movements_log 
    (user_id, room, movement_type, quantity, is_gift, to_user_id, executed_by, reason)
    VALUES (p_from_user_id, CAST(p_room AS CHAR), 'transfer_out', @to_transfer, v_is_gift, p_to_user_id, p_executed_by, 'Transferencia de cartones');
    
    -- Log entrada (CAST room para match con ENUM)
    INSERT INTO card_movements_log 
    (user_id, room, movement_type, quantity, is_gift, from_user_id, executed_by, reason)
    VALUES (p_to_user_id, CAST(p_room AS CHAR), 'transfer_in', @to_transfer, v_is_gift, p_from_user_id, p_executed_by, 'Recepción de cartones');
...
SELECT 'SP recreado con CAST en INSERT a card_movements_log' AS status;
